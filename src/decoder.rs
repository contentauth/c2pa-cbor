...
// Inner deserializer that Tagged<T> will receive in visit_newtype_struct
struct TaggedContentDeserializer<'a, R: Read> {
    de: &'a mut Decoder<R>,
    tag: u64,
}

impl<'de, 'a, R: Read> serde::Deserializer<'de> for TaggedContentDeserializer<'a, R> {
    type Error = crate::Error;

    // Forward most type deserializations to the underlying decoder, ignoring the tag
    // This allows plain types like String, i64 to deserialize normally from tagged CBOR
    serde::forward_to_deserialize_any! {
    ...
    fn deserialize_any<V: serde::de::Visitor<'de>>(self, visitor: V) -> Result<V::Value> {
        // For deserialize_any, provide a map with __tag__ and __value__
        // This is what Tagged<T> expects in visit_newtype_struct
        visitor.visit_map(TaggedMapAccess {
            de: self.de,
            tag: self.tag,
            state: TaggedMapState::BeforeTag,
    ...
    fn deserialize_struct<V: serde::de::Visitor<'de>>(
        ...
        // When deserializing as a struct (Tagged<T>'s TagAndValue helper),
        // provide the virtual tag map
        visitor.visit_map(TaggedMapAccess {
            de: self.de,
            tag: self.tag,
            state: TaggedMapState::BeforeTag,
    ...
    fn deserialize_newtype_struct<V: serde::de::Visitor<'de>>(
        ...
        // Pass through for nested newtype structs
        visitor.visit_newtype_struct(self.de)
    }
}
