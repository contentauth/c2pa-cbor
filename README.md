...
- **Opt-in deterministic (sorted-key) encoding** - RFC 8949 §4.2.1 compliant, required for C2PA manifests
...
Definite-length output alone isn't enough to make CBOR byte-for-byte reproducible: map and struct key order still depends on source order (struct field declaration order, `HashMap` iteration order, etc.), which can vary from run to run. For that, use **deterministic mode**, which additionally buffers and sorts map and struct entries by the bytewise-lexicographic order of their encoded key bytes, per RFC 8949 §4.2.1's Core Deterministic Encoding Requirement, and rejects duplicate keys. C2PA manifests require this.
